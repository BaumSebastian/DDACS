...
class BaseFEMDataset(Dataset):
    def __init__(
        ...
        if not self._data_dir.is_dir():
            raise FileNotFoundError(f"The data sub directory '{self._data_dir}' does not exist:")
    ...
    def __clean_metadata(self) -> pd.DataFrame:
        ...
        mask = self._metadata['ID'].apply(lambda id: (self._data_dir / f"{id}.h5").is_file())
        clean_metadata = self._metadata[mask]
        n_origin = len(self._metadata)
        n_clean = len(clean_metadata)

        if n_origin != n_clean:
            warnings.warn(f"Expected {n_origin} simulations but found {n_clean} in {self._data_dir}. Continues with {n_clean} simulations.", RuntimeWarning)
    ...
    def __getitem__(self, idx) -> Tuple[int, np.array, str]:
        """
        ...
        parameter = self._metadata.iloc[idx].values[1:] # The first entry is the simulation ID.
...
if __name__ == "__main__":
    ...
    print(f"ID: {type(id)} - {id}")
    print(f"Metadata: {type(p)} - {p.shape}")
    print(f"Data: {type(X)} - {X}")
...
`ddacs.streaming` is the offline-iteration namespace with four torch-free entry points:
...
- `export_to_numpy_per_sim` writes one `.npz` per simulation instead, for views whose record shapes vary.

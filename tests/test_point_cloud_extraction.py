...
class TestPointCloudExtraction:
    ...
    def test_timestep_differences(self, sample_h5_files):
        ...
        for h5_path in sample_h5_files:
            try:
                ...
                assert (
                    coords_t0.shape == coords_t1.shape
                ), f"Shape mismatch between timesteps in {h5_path}"
                ...
                assert (
                    mean_diff > 1e-10
                ), f"No movement detected between timesteps 0 and 1 in {h5_path}. Mean diff: {mean_diff}"
                assert (
                    max_diff > 1e-10
                ), f"No movement detected between timesteps 0 and 1 in {h5_path}. Max diff: {max_diff}"
                ...
                assert (
                    max_diff < 1000.0
                ), f"Suspiciously large movement detected in {h5_path}: {max_diff}"
    ...
    def test_displacement_data_extraction(self, sample_h5_files):
        """
        Test that extract_point_cloud correctly returns displacement data.

        This test verifies that the function returns raw displacement data
        from the HDF5 file without modification.
        """
        for h5_path in sample_h5_files:
            try:
                ...
                with h5py.File(h5_path, "r") as f:
                    ...
                    # Expected displacement for timestep 0 and 1
                    expected_t0 = disp[0]
                    expected_t1 = disp[1]
                    ...
                    # Check if function output matches expected displacement data
                    np.testing.assert_allclose(
                        actual_t0,
                        expected_t0,
                        rtol=1e-10,
                        err_msg=f"Timestep 0 displacement data incorrect in {h5_path}",
                    )
                    np.testing.assert_allclose(
                        actual_t1,
                        expected_t1,
                        rtol=1e-10,
                        err_msg=f"Timestep 1 displacement data incorrect in {h5_path}",
                    )
                    ...
                    assert (
                        mean_disp_diff > 1e-10
                    ), f"Displacements are identical between timesteps 0 and 1 in {h5_path}"
